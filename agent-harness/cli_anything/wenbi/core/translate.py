"""Translation operations for wenbi CLI harness."""
import os
import logging
...
def translate(input_path: str, session: Session,
              start_time: str = "", end_time: str = "") -> Dict[str, Any]:
    """
    Translate text content to a target language.

    Uses DeepL first (when available), then LLM fallback.
    ...
    if process_input is None:
        return error_result(command="translate", error="wenbi not installed. Install with: pip install wenbi")

    try:
        params = session.get_process_params()
        params["subcommand"] = "translate"
        ...
        if text_content and not text_content.startswith("Error"):
            session.record("translate", input_path, output_file or "", status="ok")
            ...
            error_msg = text_content or "Translation failed"
            session.record("translate", input_path, status="error", error=error_msg)
            return error_result(command="translate", error=error_msg)

    except Exception as e:
        session.record("translate", input_path, status="error", error=str(e))
        return error_result(command="translate", error=str(e))